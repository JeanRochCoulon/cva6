...
if [info exist GENERIC_LIST] {
  set_top r:/${TOP_LIB}/${DESIGN_NAME} -parameter $GENERIC_LIST
} else {
  set_top r:/${TOP_LIB}/${DESIGN_NAME}
}
...
# If the IP contain generic, top_name in synthesis contain generic values
if [info exist GENERIC_LIST] {
  set TOP_NAME_GENERIC [filter_collection -regexp [get_designs i:/WORK/${DESIGN_NAME}_*] "full_name =~ i:/WORK/${DESIGN_NAME}_\[0-9_\]*"]
  set_top $TOP_NAME_GENERIC
} else {
...
#
#################################################################################
# Invia-fix: Black-box for hard-coded clock gates
set hard_coded_icg [get_cells -quiet i:/WORK/*/umc55*vt_clkg]
if {$hard_coded_icg != ""} {
  set_black_box $hard_coded_icg
}

# Then hard-coded AND, NAND, INV
set hard_coded_cells [get_cells -quiet {i:/WORK/*/clkinv* i:/WORK/*/clknand* i:/WORK/*/clkand*}]
if {$hard_coded_cells != ""} {
  set_black_box $hard_coded_cells
}
